...
class GraphTransformer(nn.Module):
    ...
    def forward(self, g: gd.Batch, cond: Optional[torch.Tensor]):
        ...
        if self.num_noise > 0:
            x = torch.cat(
                [g.x, torch.rand(g.x.shape[0], self.num_noise, device=g.x.device)], 1
            )
        else:
            x = g.x
        o = self.x2h(x)
        ...
        c = self.c2h(
            cond
            if cond is not None
            else torch.ones((g.num_graphs, 1), device=g.x.device)
        )
...
class GraphTransformerGFN(nn.Module):
    ...
    def __init__(
        ...
        mlps = {}
        for atype in chain(
            env_ctx.action_type_order, env_ctx.bck_action_type_order if do_bck else []
        ):
            num_in, num_out = self._action_type_to_num_inputs_outputs[atype]
            mlps[atype.cname] = mlp(
                num_in, num_emb, num_out, cfg.model.graph_transformer.num_mlp_layers
            )
    ...
    def _make_cat(
        self, g: gd.Batch, emb: Dict[str, Tensor], action_types: list[GraphActionType]
    ):
        return GraphActionCategorical(
            g,
            raw_logits=[
                self.mlps[t.cname](emb[self._action_type_to_graph_part[t]])
    ...
    def forward(self, g: gd.Batch, cond: Optional[torch.Tensor]):
        ...
        if hasattr(g, "non_edge_index"):
            ...
            if self.edges_are_unordered:
                ...
                non_edge_embeddings = torch.cat(
                    [node_embeddings[ne_row], node_embeddings[ne_col]], 1
                )
        else:
        ...
        if self.edges_are_unordered:
            ...
            edge_embeddings = torch.cat(
                [node_embeddings[e_row], node_embeddings[e_col]], 1
            )
