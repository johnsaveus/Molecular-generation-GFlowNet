...
from utils.metrics import get_limits_of_hypercube
...
class FocusModel:
    ...
    def __init__(
        self, device: torch.device, n_objectives: int, state_space_res: int
    ) -> None:
        """
        args:
            device: torch device
...
class TabularFocusModel(FocusModel):
    ...
    def __init__(
        ...
        self.focus_dir_dataset = (
            nn.functional.normalize(
                torch.tensor(get_limits_of_hypercube(n_objectives, state_space_res)),
                dim=1,
            )
            .float()
            .to(self.device)
        )
        self.focus_dir_count = torch.zeros(self.focus_dir_dataset.shape[0]).to(
            self.device
        )
        self.focus_dir_population_count = torch.zeros(
            self.focus_dir_dataset.shape[0]
        ).to(self.device)
    ...
    def update_belief(self, focus_dirs: torch.Tensor, flat_rewards: torch.Tensor):
        ...
        focus_dirs_indices = torch.argmin(
            torch.cdist(focus_dirs, self.focus_dir_dataset), dim=1
        )
        flat_rewards_indices = torch.argmin(
            torch.cdist(flat_rewards, self.focus_dir_dataset), dim=1
        )
    ...
    def sample_focus_directions(self, n: int):
        ...
        sampling_likelihoods = (
            torch.zeros_like(self.focus_dir_count).float().to(self.device)
        )
        sampling_likelihoods[self.focus_dir_count == 0] = self.feasible_flow
        sampling_likelihoods[
            torch.logical_and(
                self.focus_dir_count > 0, self.focus_dir_population_count > 0
            )
        ] = self.feasible_flow
        sampling_likelihoods[
            torch.logical_and(
    ...
    def load(self, device: torch.device, path: Path):
        ...
        self.focus_dir_population_count = params["focus_dir_population_count"].to(
            device
        )
