...
def compute_focus_coef(
    ...
    """
    assert (
        focus_cosim >= 0.0 and focus_cosim <= 1.0
    ), f"focus_cosim must be in [0, 1], now {focus_cosim}"
    assert (
        focus_limit_coef > 0.0 and focus_limit_coef <= 1.0
    ), f"focus_limit_coef must be in (0, 1], now {focus_limit_coef}"
    focus_gamma_param = torch.tensor(
        np.log(focus_limit_coef) / np.log(focus_cosim)
    ).float()
...
def get_focus_accuracy(flat_rewards, focus_dirs, focus_cosim):
    _, in_focus_mask = compute_focus_coef(
        focus_dirs, flat_rewards, focus_cosim, focus_limit_coef=1.0
    )
...
def partition_hypersphere(
    k: int, d: int, n_samples: int = 10000, normalisation: str = "l2"
):
    """
...
def _sharpeRatioQPMax(p, Q, rf):
    ...
    sol = solvers.coneqp(
        G, c, -C, -d, None, A, b, kktsolver="ldl"
    )  # , initvals=self.initGuess)
    y = np.array(sol["x"])
...
def HSRindicator(A, low, up, managedup=False):
    ...
    if (up <= low).any():
        raise ValueError(
            "The lower reference point does not strongly dominate the upper reference point!"
        )
...
class HSR_Calculator:
    ...
    def calculate_hsr(self, solutions):
        ...
        hsr_indicator, hsr_invest = HSRindicator(
            A=max_solutions, low=self.lower_bound, up=self.upper_bound
        )
...
if __name__ == "__main__":
    ...
    A = np.array(
        [[1, 3], [2, 2], [3, 1]]
    )  # matrix with dimensions n x d (n points, d dimensions)
