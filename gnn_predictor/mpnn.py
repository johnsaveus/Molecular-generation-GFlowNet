...
class GraphTransformer(nn.Module):
    def __init__(
        ...
        self.gnn_layers.append(
            TransformerConv(input_dim, hidden_dims[0], heads[0], edge_dim=edge_dim)
        )
    ...
    def forward(
        self, x: Tensor, edge_index: Tensor, edge_attr: Optional[Tensor], batch: Tensor
    ) -> Tensor:
...
def bond_features(bond, use_chirality=False):
    ...
    if use_chirality:
        bond_feats = bond_feats + one_of_k_encoding_unk(
            str(bond.GetStereo()), ["STEREONONE", "STEREOZ", "STEREOE"]
        )
