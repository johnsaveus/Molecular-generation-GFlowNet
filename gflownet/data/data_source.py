...
class DataSource(IterableDataset):
    ...
    def do_sample_model(self, model, num_from_policy, num_new_replay_samples=None):
        if num_new_replay_samples is not None:
            ...
            ), "num_new_replay_samples specified without a replay buffer"
        if num_new_replay_samples is None:
            assert (
                self.replay_buffer is None
            ), "num_new_replay_samples not specified with a replay buffer"
        ...
        def iterator():
            while self.active:
                ...
                trajs = self.algo.create_training_data_from_own_samples(
                    model, num_samples, cond_info["encoding"], p
                )
                self.set_traj_cond_info(
                    trajs, cond_info
                )  # Attach the cond info to the trajs
                self.compute_properties(trajs, mark_as_online=True)
                self.compute_log_rewards(trajs)
                self.send_to_replay(
                    trajs[:num_new_replay_samples]
                )  # This is a no-op if there is no replay buffer
    ...
    def do_sample_model_n_times(self, model, num_samples_per_batch, num_total):
        ...
        def iterator():
            while self.active:
                ...
                # TODO: in the cond info refactor, pass the whole thing instead of just the encoding
                trajs = self.algo.create_training_data_from_own_samples(
                    model, n_this_time, cond_info["encoding"], p
                )
                self.set_traj_cond_info(
                    trajs, cond_info
                )  # Attach the cond info to the trajs
    ...
    def do_sample_replay(self, num_samples):
        def iterator():
            while self.active:
                ...
                self.relabel_in_hindsight(
                    trajs
                )  # This is a no-op if the hindsight ratio is 0
                yield trajs, {}
    ...
    def do_dataset_in_order(self, data, num_samples, backwards_model):
        def iterator():
            for idcs in self.iterate_indices(len(data), num_samples):
                ...
                cond_info = self.task.sample_conditional_information(num_samples, t)
                objs, props = (
                    map(list, zip(*[data[i] for i in idcs])) if len(idcs) else ([], [])
                )
                trajs = self.algo.create_training_data_from_graphs(
                    objs, backwards_model, cond_info["encoding"], p
                )
                self.set_traj_cond_info(
                    trajs, cond_info
                )  # Attach the cond info to the trajs
    ...
    def do_conditionals_dataset_in_order(self, data, num_samples, model):
        def iterator():
            for idcs in self.iterate_indices(len(data), num_samples):
                ...
                trajs = self.algo.create_training_data_from_own_samples(
                    model, len(idcs), cond_info["encoding"], p
                )
                self.set_traj_cond_info(
                    trajs, cond_info
                )  # Attach the cond info to the trajs
    ...
    def call_sampling_hooks(self, trajs):
        ...
        cond_info = {
            k: torch.stack([t["cond_info"][k] for t in trajs])
            for k in trajs[0]["cond_info"]
        }
    ...
    def create_batch(self, trajs, batch_info):
        ...
        if "preferences" in trajs[0]["cond_info"].keys():
            batch.preferences = torch.stack(
                [t["cond_info"]["preferences"] for t in trajs]
            )
    ...
    def compute_properties(self, trajs, mark_as_online=False):
        ...
        valid_idcs = torch.tensor(
            [i for i in range(len(trajs)) if trajs[i].get("is_valid", True)]
        ).long()
        ...
        if self.replay_buffer is not None:
            for t in trajs:
                self.replay_buffer.push(
                    t, t["log_reward"], t["obj_props"], t["cond_info"], t["is_valid"]
                )
    ...
    def relabel_in_hindsight(self, trajs):
        ...
        hindsight_idxs = torch.randperm(len(trajs))[
            : int(len(trajs) * self.cfg.replay.hindsight_ratio)
        ]
        log_rewards = torch.stack([t["log_reward"] for t in trajs])
        obj_props = torch.stack([t["obj_props"] for t in trajs])
        cond_info = {
