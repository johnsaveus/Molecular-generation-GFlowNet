...
class SQLiteLogHook:
    def __init__(self, log_dir, ctx) -> None:
        self.log = (
            None  # Only initialized in __call__, which will occur inside the worker
        )
    ...
    def __call__(self, trajs, rewards, obj_props, cond_info):
        ...
        preferences = (
            cond_info.get("preferences", torch.zeros((len(objs), 0)))
            .data.numpy()
            .tolist()
        )
        focus_dir = (
            cond_info.get("focus_dir", torch.zeros((len(objs), 0)))
            .data.numpy()
            .tolist()
        )
        logged_keys = [
            k
            for k in sorted(cond_info.keys())
            if k not in ["encoding", "preferences", "focus_dir"]
        ]
...
def read_all_results(path):
    ...
    dfs = [
        pd.read_sql_query(
            "SELECT * FROM results",
            sqlite3.connect(f"file:{path}/generated_objs_{i}.db?mode=ro"),
        )
