...
class MultiObjectiveConfig(StrictDataClass):
    num_objectives: int = (
        2  # TODO: Change that as it can conflict with cfg.task.seh_moo.num_objectives
    )
...
class ConditionalsConfig(StrictDataClass):
    ...
    weighted_prefs: WeightedPreferencesConfig = field(
        default_factory=WeightedPreferencesConfig
    )
