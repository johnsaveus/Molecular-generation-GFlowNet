...
class MultiObjectiveWeightedPreferences(Conditional[ObjectProperties, LinScalar]):
    def __init__(self, cfg: Config):
        ...
        if self.cfg.preference_type == "seeded":
            self.seeded_prefs = np.random.default_rng(142857 + int(cfg.seed)).dirichlet(
    ...
    def sample(self, n):
        if self.cfg.preference_type is None:
            ...
            a = np.random.dirichlet(
                [self.cfg.preference_param] * self.num_objectives, n
            )
            ...
            m = Dirichlet(
                torch.FloatTensor([self.cfg.preference_param] * self.num_objectives)
            )
    ...
    def transform(
        self, cond_info: Dict[str, Tensor], flat_reward: ObjectProperties
    ) -> LinScalar:
        scalar_reward = (flat_reward * cond_info["preferences"]).sum(1)
        assert (
            len(scalar_reward.shape) == 1
        ), f"scalar_reward should be a 1D array, got {scalar_reward.shape}"
    ...
    def encode(self, conditional: Tensor) -> Tensor:
        if self.num_thermometer_dim > 0:
            return thermometer(conditional, self.num_thermometer_dim, 0, 1).reshape(
                conditional.shape[0], -1
            )
        else:
...
class FocusRegionConditional(
    Conditional[tuple[ObjectProperties, LogScalar], LogScalar]
):
    def __init__(self, cfg: Config, n_valid: int):
        ...
        self.ocfg = cfg
        self.num_thermometer_dim = (
            cfg.cond.moo.num_thermometer_dim if self.cfg.use_steer_thermomether else 0
        )
    ...
    def setup_focus_regions(self):
        ...
        if self.cfg.focus_type is None:
            ...
            valid_focus_dirs = metrics.partition_hypersphere(
                d=self.n_objectives, k=self.n_valid, normalisation="l2"
            )
            self.fixed_focus_dirs = valid_focus_dirs
        elif self.cfg.focus_type in ["dirichlet", "learned-gfn"]:
            valid_focus_dirs = metrics.partition_hypersphere(
                d=self.n_objectives, k=self.n_valid, normalisation="l1"
            )
    ...
    def sample(self, n: int, train_it: Optional[int] = None):
        ...
        if self.fixed_focus_dirs is not None:
            focus_dir = torch.tensor(
                np.array(self.fixed_focus_dirs)[
                    rng.choice(len(self.fixed_focus_dirs), n)
                ].astype(np.float32)
            )
            ...
            focus_dir = torch.tensor(
                metrics.sample_positiveQuadrant_ndim_sphere(
                    n, self.n_objectives, normalisation="l2"
                )
            ).float()
            ...
            if (
                ...
                and train_it
                >= self.cfg.focus_model_training_limits[0] * self.cfg.max_train_it
            ):
            ...
            raise NotImplementedError(
                f"Unsupported focus_type={type(self.cfg.focus_type)}"
            )
    ...
    def encode(self, conditional: Tensor) -> Tensor:
        return (
            thermometer(
                conditional, self.ocfg.cond.moo.num_thermometer_dim, 0, 1
            ).reshape(conditional.shape[0], -1)
            if self.cfg.use_steer_thermomether
            else conditional
        )

    def transform(
        self, cond_info: Dict[str, Tensor], data: tuple[ObjectProperties, LogScalar]
    ) -> LogScalar:
        flat_rewards, scalar_logreward = data
        focus_coef, in_focus_mask = metrics.compute_focus_coef(
            flat_rewards,
            cond_info["focus_dir"],
            self.cfg.focus_cosim,
            self.cfg.focus_limit_coef,
        )
        scalar_logreward = LogScalar(
            scalar_logreward.clone()
    ...
    def step_focus_model(self, batch: gd.Batch, train_it: int):
        ...
        if (
            ...
            self.focus_model.update_belief(
                deepcopy(batch.focus_dir), deepcopy(batch.flat_rewards)
            )
