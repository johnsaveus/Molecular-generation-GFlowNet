...
class GraphAction:
    def __init__(
        self, action: GraphActionType, source=None, target=None, value=None, attr=None
    ):
...
class GraphBuildingEnv:
    ...
    def step(self, g: Graph, action: GraphAction) -> Graph:
        ...
        if action.action is GraphActionType.AddEdge:
            ...
            assert (
                action.attr not in gp.nodes[action.source]
                or gp.nodes[action.source][action.attr] is None
            )
            ...
            assert g.has_edge(action.source, action.target)
            gp = graph_without_edge_attr(
                gp, (action.source, action.target), action.attr
            )
        else:
    ...
    def parents(self, g: Graph):
        ...
        for a, b in g.edges:
            if degree[a] > 1 and degree[b] > 1 and len(g.edges[(a, b)]) == 0:
                ...
                if nx.algorithms.is_connected(new_g):
                    add_parent(
                        GraphAction(GraphActionType.AddEdge, source=a, target=b), new_g
                    )
        ...
        for i in g.nodes:
            ...
            if len(g.nodes) == 1 and len(g.nodes[i]) == 1:
                ...
                add_parent(
                    GraphAction(
                        GraphActionType.AddNode, source=0, value=g.nodes[i]["v"]
                    ),
    ...
    def reverse(self, g: Graph, ga: GraphAction):
        if ga.action == GraphActionType.Stop:
            ...
            return GraphAction(
                GraphActionType.RemoveEdge, source=ga.source, target=ga.target
            )
        elif ga.action == GraphActionType.SetNodeAttr:
            return GraphAction(
                GraphActionType.RemoveNodeAttr, source=ga.source, attr=ga.attr
            )
            ...
            assert (
                len(neighbors) <= 1
            )  # RemoveNode should only be a legal action if the node has one or zero neighbors
            source = (
                0
                if not len(neighbors)
                else neighbors[0][0]
                if neighbors[0][0] != ga.source
                else neighbors[0][1]
            )
            return GraphAction(
                GraphActionType.AddNode, source=source, value=g.nodes[ga.source]["v"]
            )
        elif ga.action == GraphActionType.RemoveEdge:
            return GraphAction(
                GraphActionType.AddEdge, source=ga.source, target=ga.target
            )
...
class GraphActionCategorical:
    def __init__(
        ...
        self.slice = [
            slice_dict[k].to(dev)
            if k is not None
            else torch.arange(graphs.num_graphs + 1, device=dev)
            for k in keys
        ]
        self.logprobs = None
        self.log_n = None
    ...
    def _apply_action_masks(self):
        self._masked_logits = (
            [
                self._mask(logits, mask)
                for logits, mask in zip(self.raw_logits, self._action_masks)
            ]
    ...
    def _compute_batchwise_max(
        ...
        outs = [
            torch.zeros(self.num_graphs, i.shape[1], device=self.dev) + min_val
            for i in x
        ]
    ...
    def logsoftmax(self):
        ...
        corr_logits = [
            (i - maxl[b, None]) for i, b in zip(self._masked_logits, self.batch)
        ]
        exp_logits = [
            i.exp().clamp(self._epsilon) for i, b in zip(corr_logits, self.batch)
        ]
    ...
    def logsumexp(self, x=None):
        ...
        # x[b, None] indexes by the batch to map back to each node/edge and adds a broadcast dim
        exp_vals = [
            (i - maxl[b, None]).exp().clamp(self._epsilon)
            for i, b in zip(x, self.batch)
        ]
    ...
    def sample(self) -> List[ActionIndex]:
        ...
        # Gumbel noise
        gumbel = [
            logit - (-noise.log()).log() for logit, noise in zip(self._masked_logits, u)
        ]
        # Take the argmax
    ...
    def argmax(
        ...
        for i in range(type_max_idx.shape[0]):
            ...
            argmaxes.append(
                ActionIndex(
                    action_type=int(t),
                    row_idx=int(row_pos[t][i] - self.slice[t][i]),
                    col_idx=int(col_max[t][1][i]),
                )
            )
    ...
    def log_prob(
        ...
        assert (
            actions.shape[0] == batch.shape[0]
        )  # Check there are as many actions as batch indices
        ...
        t_offsets = torch.tensor(
            [0] + [i.numel() for i in logprobs], device=self.dev
        ).cumsum(0)[actions[:, 0]]
...
class GraphBuildingEnvContext:
    ...
    def ActionIndex_to_GraphAction(
        self, g: gd.Data, aidx: ActionIndex, fwd: bool = True
    ) -> GraphAction:
    ...
    def GraphAction_to_ActionIndex(
    ...
    def object_to_log_repr(self, g: Graph) -> str:
        ...
        return json.dumps(
            [[(i, g.nodes[i]) for i in g.nodes], [(e, g.edges[e]) for e in g.edges]],
            separators=(",", ":"),
        )
...
def action_type_to_mask(
    t: GraphActionType, gbatch: gd.Batch, assert_mask_exists: bool = False
):
    if assert_mask_exists:
        assert hasattr(
            gbatch, t.mask_name
        ), f"Mask {t.mask_name} not found in graph data"
    return (
        getattr(gbatch, t.mask_name)
        if hasattr(gbatch, t.mask_name)
        else torch.ones((1, 1), device=gbatch.x.device)
    )
