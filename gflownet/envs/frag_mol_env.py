...
class FragMolBuildingEnvContext(GraphBuildingEnvContext):
    ...
    def __init__(
        ...
        max_frags: int = 9,
        num_cond_dim: int = 0,
        fragments: List[Tuple[str, List[int]]] = None,
    ):
        ...
        self.sorted_frags = sorted(
            list(enumerate(self.frags_mol)), key=lambda x: -x[1].GetNumAtoms()
        )
    ...
    def graph_to_Data(self, g: Graph) -> gd.Data:
        ...
        remove_node_mask = (
            ones((x.shape[0], 1)) if len(g) == 0 else zeros((x.shape[0], 1))
        )
        remove_edge_attr_mask = zeros((len(g.edges), self.num_edge_attrs))
        if len(g):
            degrees = np.array(list(g.degree), dtype=np.int32)[:, 1]  # type: ignore
            max_degrees = np.array(
                [len(self.frags_stems[g.nodes[n]["v"]]) for n in g.nodes]
            )  # type: ignore
        else:
        ...
        if len(g):
            remove_node_mask = (
                node_is_connected * (1 - node_is_connected_to_edge_with_attr)
            ).reshape((-1, 1))
                ...
                if attach_name not in ad:
                    for attach_point in range(max_degrees[n]):
                        if attach_point not in attached[n]:
                            set_edge_attr_mask[
                                i, attach_point + self.num_stem_acts * j
                            ] = 1
        # Since this is a DiGraph, make sure to put (i, j) first and (j, i) second
        edge_index = (
            np.array([e for i, j in g.edges for e in [(i, j), (j, i)]], dtype=np.int64)
            .reshape((-1, 2))
            .T
        )
        ...
        if x.shape[0] == self.max_frags:
            ...
            add_node_mask = add_node_mask * np.ones(
                (x.shape[0], self.num_new_node_values), np.float32
            )
    ...
    def graph_to_obj(self, g: Graph) -> Chem.Mol:
        ...
        for a, b in g.edges:
            ...
            if self.fail_on_missing_attr:
                assert (
                    "src_attach" in g.edges[(a, b)] and "dst_attach" in g.edges[(a, b)]
                )
            u, v = (
                int(
                    self.frags_stems[afrag][g.edges[(a, b)].get("src_attach", 0)]
                    + offsets[a]
                ),
                int(
                    self.frags_stems[bfrag][g.edges[(a, b)].get("dst_attach", 0)]
                    + offsets[b]
                ),
            )
            bond_atoms += [u, v]
        ...
        while len(q) > 0:  # print(i, x)
            ...
            for i in nx.neighbors(og, x):
                if not visited[i]:
                    visited[i] = True
                    g.add_edge(
                        x, i, **(og.get_edge_data(x, i) | og.get_edge_data(i, x))
                    )
                    q.append(i)
...
def _recursive_decompose(
    ctx, m, all_matches, a2f, frags, bonds, max_depth=9, numiters=None
):
    if numiters is None:
        numiters = [0]
    numiters[0] += 1
    ...
    if max_depth == 0 or len(a2f) == m.GetNumAtoms():
        ...
        for a, b, stemidx_a, stemidx_b, _, _ in bonds:
            g.edges[(a, b)]["src_attach"] = (
                stemidx_a  # TODO: verify src/dst is correct?
            )
    ...
    for fragidx, frag in ctx.sorted_frags:
        ...
        for match in matches:
            ...
            if any(
                frag.GetAtomWithIdx(ai).GetFormalCharge()
                != m.GetAtomWithIdx(bi).GetFormalCharge()
            ...
            for fi, i in enumerate(match):
                for j in m.GetAtomWithIdx(i).GetNeighbors():
                    ...
                    if (
                        m.GetBondBetweenAtoms(i, j).GetBondType()
            ...
            if not is_valid_match:
                continue
            for this_frag_stemidx, i in enumerate(
                [match[s] for s in ctx.frags_stems[fragidx]]
            ):
                for j in m.GetAtomWithIdx(i).GetNeighbors():
                    j = j.GetIdx()
                    if j in match:
                    ...
                    if j in a2f and a2f[j] != new_frag_idx:
                        ...
                        try:
                            ...
                            other_frag_stemidx = ctx.frags_stems[
                                frags[other_frag_idx]
                            ].index(other_frag_atomidx)
                        except ValueError:
                            continue
