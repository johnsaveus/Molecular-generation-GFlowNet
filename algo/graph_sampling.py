...
class GraphSampler:
    ...
    def sample_from_model(
        ...
        for t in range(self.max_len):
            ...
            fwd_cat, *_, log_reward_preds = model(
                self.ctx.collate(torch_graphs).to(dev), ci
            )
            if random_action_prob > 0:
                ...
                fwd_cat.logits = [
                    is_random_action[b][:, None] * torch.ones_like(i) * 100
            ...
            for i, j in zip(not_done(range(n)), range(n)):
                ...
                if (
                    ...
                    and self.sanitize_samples
                    and not self.ctx.is_sane(graphs[i])
                ):
    ...
    def sample_backward_from_graphs(
        ...
        while sum(done) < n:
            torch_graphs = [
                self.ctx.graph_to_Data(graphs[i]) for i in not_done(range(n))
            ]
            not_done_mask = torch.tensor(done, device=dev).logical_not()
            if model is not None:
                ...
                action_masks = [
                    action_type_to_mask(t, gbatch, assert_mask_exists=True)
                ...
                bck_cat = GraphActionCategorical(
                    ...
                    keys=[
                        GraphTransformerGFN.action_type_to_key(t) for t in action_types
                    ],
            ...
            graph_bck_actions = [
                self.ctx.ActionIndex_to_GraphAction(g, a, fwd=False)
                for g, a in zip(torch_graphs, bck_actions)
            ]
        ...
        for i in range(n):
            ...
            data[i]["bck_a"] = [GraphAction(GraphActionType.Stop)] + data[i]["bck_a"][
                ::-1
            ]
            data[i]["is_sink"] = data[i]["is_sink"][::-1]
            data[i]["bck_logprobs"] = torch.tensor(
                data[i]["bck_logprobs"][::-1], device=dev
            ).reshape(-1)
            if self.pad_with_terminal_state:
                data[i]["traj"].append((graphs[i], GraphAction(GraphActionType.Stop)))
                data[i]["is_sink"].append(1)
