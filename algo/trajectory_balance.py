...
class TrajectoryBalance(GFNAlgorithm):
    ...
    def __init__(
        ...
        assert (
            self.cfg.backward_policy not in [Backward.Maxent, Backward.GSQL]
            or self.cfg.n_loss != NLoss.none
        ...
        assert (
            self.cfg.do_predict_n or self.cfg.n_loss == NLoss.none
        ), "`n_loss != NLoss.none` requires `do_predict_n`"
        self.random_action_prob = [
            cfg.algo.train_random_action_prob,
            cfg.algo.valid_random_action_prob,
        ]
    ...
    def create_training_data_from_own_samples(
        ...
        data = self.graph_sampler.sample_from_model(
            model, n, cond_info, random_action_prob
        )
    ...
    def create_training_data_from_graphs(
        ...
        trajs: List[Dict[str, Any]] = [
            {"traj": generate_forward_trajectory(i)} for i in graphs
        ]
        for traj in trajs:
            n_back = [
            ...
            traj["bck_logprobs"] = (
                (1 / torch.tensor(n_back).float()).log().to(get_worker_device())
            )
            traj["result"] = traj["traj"][-1][0]
            if self.cfg.do_parameterize_p_b:
                traj["bck_a"] = [GraphAction(GraphActionType.Stop)] + [
                    self.env.reverse(g, a) for g, a in traj["traj"]
                ]
    ...
    def get_idempotent_actions(
        ...
        for i in nz:
            aidx = ActionIndex(
                action_type=iaction[0], row_idx=i[0].item(), col_idx=i[1].item()
            )
            if aidx == iaction:
                continue
            ga = self.ctx.ActionIndex_to_GraphAction(
                gd, aidx, fwd=not action.action.is_backward
            )
            child = self.env.step(g, ga)
            if nx.algorithms.is_isomorphic(
                child, gp, lambda a, b: a == b, lambda a, b: a == b
            ):
    ...
    def construct_batch(self, trajs, cond_info, log_rewards):
        ...
        if self.model_is_autoregressive:
            ...
            actions = [
                self.ctx.GraphAction_to_ActionIndex(g, i[1])
                for g, tj in zip(torch_graphs, trajs)
                for i in tj["traj"]
            ]
        else:
            torch_graphs = [
            ...
            actions = [
                ...
                for g, a in zip(
                    torch_graphs, [i[1] for tj in trajs for i in tj["traj"]]
                )
            ]
        ...
        if self.cfg.do_parameterize_p_b:
            batch.bck_actions = torch.tensor(
                [
                    self.ctx.GraphAction_to_ActionIndex(g, a)
                    for g, a in zip(
                        torch_graphs, [i for tj in trajs for i in tj["bck_a"]]
                    )
                ]
            )
        ...
        if self.cfg.do_correct_idempotent:
            ...
            bgraphs = sum(
                [[i[0] for i in tj["traj"][1:]] + [tj["result"]] for tj in trajs], []
            )
            ...
            if self.cfg.do_parameterize_p_b:
                ...
                bgraphs = sum(
                    [[None] + [i[0] for i in tj["traj"][:-1]] for tj in trajs], []
                )
    ...
    def compute_batch_losses(
        ...
        if self.ctx.has_n() and self.cfg.do_predict_n:
            analytical_maxent_backward = self.analytical_maxent_backward(
                batch, first_graph_idx
            )
            if self.cfg.do_parameterize_p_b:
                analytical_maxent_backward = torch.roll(
                    analytical_maxent_backward, -1, 0
                ) * (1 - batch.is_sink)
        else:
        ...
        traj_unif_log_p_B = scatter(
            log_p_B_unif, batch_idx, dim=0, dim_size=num_trajs, reduce="sum"
        )
        ...
        if self.ctx.has_n() and self.cfg.do_predict_n:
            info["n_loss_pred"] = scatter(
                (log_n_preds - batch.log_ns) ** 2,
                batch_idx,
                dim=0,
                dim_size=num_trajs,
                reduce="sum",
            ).mean()
            info["n_final_loss"] = torch.mean(
                (log_n_preds[final_graph_idx] - batch.log_n) ** 2
            )
            if self.cfg.do_parameterize_p_b:
                ...
                d = d * d
                d[final_graph_idx] = 0
                info["n_loss_maxent"] = scatter(
                    d, batch_idx, dim=0, dim_size=num_trajs, reduce="sum"
                ).mean()
    ...
    def _init_subtb(self, dev):
        ...
        tidx = [
            torch.tril_indices(i, i, device=dev)[1] for i in range(self._subtb_max_len)
        ]
        ...
        self._precomp = [
            (
                ...
                torch.cat(
                    [
                        ar[: T - i].repeat_interleave(ar[: T - i] + 1)
                        + ar[T - i + 1 : T + 1].sum()
                        for i in range(T)
                    ]
                ),
    ...
    def subtb_loss_fast(self, P_F, P_B, F, R, traj_lengths):
        ...
        cumul_lens = torch.cumsum(
            torch.cat([torch.zeros(1, device=dev), traj_lengths]), 0
        ).long()
        ...
        for ep in range(traj_lengths.shape[0]):
            ...
            fidces = torch.cat(
                [
                    torch.cat(
                        [
                            ar[i + 1 : T] + offset,
                            torch.tensor([R_start + ep], device=dev),
                        ]
                    )
                    for i in range(T)
                ]
            ...
            total_loss[ep] = (
                self._loss(F_start - F_end + P_F_sums - P_B_sums).sum() / car[T]
            )
        return total_loss
    ...
    def n_loss(self, P_N, N, traj_lengths):
        ...
        for ep, (s_idx, e_idx) in enumerate(zip(shift_right(x), x)):
            ...
            e_idx -= 1
            total_loss[ep] = self._n_loss(
                self.cfg.n_loss, P_N[s_idx : e_idx - 1], N[s_idx:e_idx]
            )
        return total_loss
