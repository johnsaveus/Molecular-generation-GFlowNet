...
class GFNTrainer:
    ...
    def build_training_data_loader(self) -> DataLoader:
        ...
        if self.cfg.replay.use:
            ...
            assert (
                self.cfg.replay.num_from_replay != 0
            ), "Replay is enabled but no samples are being drawn from it"
            assert (
                self.cfg.replay.num_new_samples != 0
            ), "Replay is enabled but no new samples are being added to it"

        n_drawn = self.cfg.algo.num_from_policy
        n_replayed = (
            self.cfg.replay.num_from_replay or n_drawn if self.cfg.replay.use else 0
        )
        n_new_replay_samples = (
            self.cfg.replay.num_new_samples or n_drawn if self.cfg.replay.use else None
        )
        n_from_dataset = self.cfg.algo.num_from_dataset

        src = DataSource(
            self.cfg, self.ctx, self.algo, self.task, replay_buffer=replay_buffer
        )
        if n_from_dataset:
            src.do_sample_dataset(
                self.training_data, n_from_dataset, backwards_model=model
            )
        if n_drawn:
        ...
        if self.cfg.log_dir:
            src.add_sampling_hook(
                SQLiteLogHook(str(pathlib.Path(self.cfg.log_dir) / "train"), self.ctx)
            )
    ...
    def build_validation_data_loader(self) -> DataLoader:
        ...
        if n_from_dataset:
            src.do_dataset_in_order(
                self.test_data, n_from_dataset, backwards_model=model
            )
        if n_drawn:
            ...
            src.do_sample_model_n_times(
                model, n_drawn, num_total=self.cfg.num_validation_gen_steps * n_drawn
            )

        if self.cfg.log_dir:
            src.add_sampling_hook(
                SQLiteLogHook(str(pathlib.Path(self.cfg.log_dir) / "valid"), self.ctx)
            )
    ...
    def build_final_data_loader(self) -> DataLoader:
        ...
        # TODO: might be better to change total steps to total trajectories drawn
        src.do_sample_model_n_times(
            model, n_drawn, num_total=self.cfg.num_final_gen_steps * n_drawn
        )

        if self.cfg.log_dir:
            src.add_sampling_hook(
                SQLiteLogHook(str(pathlib.Path(self.cfg.log_dir) / "final"), self.ctx)
            )
        for hook in self.sampling_hooks:
            src.add_sampling_hook(hook)
        return self._make_data_loader(src)
    ...
    def run(self, logger=None):
        ...
        ckpt_freq = (
            ...
            if self.cfg.checkpoint_every is not None
            else valid_freq
        )
        ...
        if num_final_gen_steps:
            ...
            for it, batch in zip(
                range(
                    num_training_steps + 1, num_training_steps + num_final_gen_steps + 1
                ),
                cycle(final_dl),
            ):
                if hasattr(batch, "extra_info"):
                ...
                if it % self.print_every == 0:
                    logger.info(
                        f"Generating objs {it - num_training_steps}/{num_final_gen_steps}"
                    )
            final_info = {k: np.mean(v) for k, v in final_info.items()}

            logger.info(
                "Final generation steps completed - "
